...
namespace DaniDojo.Managers
{
    internal class DaniPlayManager
    {
        ...
        static public bool HasFailed()
        {
            ...
            //ModLogger.Log("HasFailed: songZNum " + songNum, 2);

            for (int i = 0; i < currentCourse.Borders.Count; i++)
            {
                var border = currentCourse.Borders[i];
                //ModLogger.Log("HasFailed: Border " + border.BorderType.ToString(), 2);
                ...
                if (border.IsTotal)
                {
                    ...
                    {
                        //ModLogger.Log("HasFailed: ReqReqs.Sum(): " + border.RedReqs.Sum(), 2);
                        //ModLogger.Log("HasFailed: Value.Sum(): " + values.Sum(), 2);
                ...
                {
                    //ModLogger.Log("HasFailed: RedReqs[songNum]: " + border.RedReqs[songNum], 2);
                    //ModLogger.Log("HasFailed: values[songNum]: " + values[songNum], 2);
                    if (border.BorderType == BorderType.Oks ||
                        border.BorderType == BorderType.Bads)
                    {
        ...
        {
            //ModLogger.Log("CheckIsInDan: " + IsInDan, 5);
            return IsInDan;
        }

        static public void StartDanPlay(DaniCourse course)
        {
            ModLogger.Log("Start Course " + course.Parent.Title + " - " + course.Title, 0);
            IsInDan = true;
            StartResult = false;
        ...
        static public void RestartDanPlay()
        {
            if (IsInDan)
            {
                //ModLogger.Log("Restart Course", 1);
        ...
        {
            //ModLogger.Log("Score added: " + points, 2);
        ...
        {
            //ModLogger.Log("CalculateRankBorders Start", 2);
            if (course == null)
            {
                return DaniRank.None;
            }
            //ModLogger.Log("CalculateRankBorders: Series: " + course.Parent.Title, 2);
            //ModLogger.Log("CalculateRankBorders: Course: " + course.Title, 2);
            ...
            for (int i = 0; i < course.Borders.Count; i++)
            {
                ...
                //ModLogger.Log("CalculateRankBorders: Border " + i + " (" + course.Borders[i].BorderType.ToString() + "): " + minRank.ToString(), 2);
        ...
        static public DaniRank CalculateBorderMidSong(DaniBorder border)
        {
            ...
            {
                //ModLogger.Log(LogType.Error, "CalculateBorder Error: Too many values in PlayValues compared to Border Requirements.");
                return DaniRank.None;
            }
        ...
        static DaniRank CalculateBorder(DaniBorder border, PlayData play)
        {
            ...
            //ModLogger.Log("CalculateBorder: BorderType: " + border.BorderType.ToString(), 2);
            for (int i = 0; i < playValues.Count; i++)
            {
                //ModLogger.Log("CalculateBorder: playValues[" + i + "]: " + playValues[i].ToString(), 2);
                //ModLogger.Log("CalculateBorder: border.RedReqs[" + i + "]: " + border.RedReqs[i].ToString(), 2);
                //ModLogger.Log("CalculateBorder: border.GoldReqs[" + i + "]: " + border.GoldReqs[i].ToString(), 2);
                if (border.BorderType == BorderType.Oks ||
                    border.BorderType == BorderType.Bads)
                {
        ...
        {
            //ModLogger.Log("CalculateComboRank Start", 2);
            if (play.SongReached != play.SongPlayData.Count)
            {
                //ModLogger.Log("CalculateComboRank DaniCombo.None", 2);
            ...
            {
                //ModLogger.Log("CalculateComboRank DaniCombo.Rainbow", 2);
                return DaniCombo.Rainbow;
            }
            else if (numBads == 0)
            {
                //ModLogger.Log("CalculateComboRank DaniCombo.Gold", 2);
                return DaniCombo.Gold;
            }
            else
            {
                //ModLogger.Log("CalculateComboRank DaniCombo.Silver", 2);
